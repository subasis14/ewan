...
Class corporatesectorController Extends baseController {
    ...
    public function index() {
        ...
        $adminEmail=$formObj->getAdminEmail();
            ...
            if ($_POST['ajax'] == 1) {
                if ($_POST['reqType'] == 'insertCorporateForm') {
                ...
                $result = $formObj->insertCorporateformData($fistName, $family_name, $email, $mobile, $gender, $profession, $city, $nationalities, $requiredPeriod);
                if ($result) {
                   $userSubject="Acknowledgment from Ewam Team";
                        $adminSubject="New Enquiry from Corporate Sector";
                ...
                if ($_POST['reqType'] == 'statusUpdate') {
                    $status = $_POST['status'];
                    $row_id = $_POST['rowId'];
                    $result = $formObj->updateCorporateStatusById($status, $row_id);
            ...
            $this->registry->template->show('services/corporatesector');
        }
    }
