...
Class healthcaresectorController Extends baseController {
    ...
    public function index() {
        ...
        $adminEmail=$formObj->getAdminEmail();
            ...
            if ($_POST['ajax'] == 1) {
                if ($_POST['reqType'] == 'insertHealthcareForm') {
                ...
                $result = $formObj->insertHealthcareFormData($fistName, $family_name, $email, $mobile, $gender, $profession, $city, $nationalities, $requiredPeriod);
                if ($result) {
                        $userSubject="Acknowledgment from Ewam Team";
                        $adminSubject="New Enquiry from Healthcare Sector";
                ...
                if ($_POST['reqType'] == 'statusUpdate') {
                    $status = $_POST['status'];
                    $row_id = $_POST['rowId'];
                    $result = $formObj->updateHealthcareStatusById($status, $row_id);
            ...
            $this->registry->template->show('services/healthcaresector');
        }
    }
